...
class CalmMindSupabaseUser extends BaseAuthUser {
  CalmMindSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> calmMindSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = CalmMindSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
